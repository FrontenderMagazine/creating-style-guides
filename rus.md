...
все элементы и модули сайта или приложения. Сверх консолидации кода, оно ещё и
...
от владельца, до дизайнеров и разработчиков, при обсуждении внесения изменений и 
итераций создания продукта. Некоторые компании даже выкладывают руководства по 
...
(Надо, однако, заметить, что то, что я называю *руководство по стилю*, некоторые
люди называют *библиотекой шаблонов*. Большинство руководство используют термин, 
*руководство по стилю*, но термин *библиотека шаблонов* набирает популярность.)
...
используется и задокументировать её для моего и команды дальнейшего использования.
В дополнение к отлову непоследовательностей и ошибок, что непременно происходит
...
по объему и быстро загружающимся. Используя руководство как справочник модулей и
кода, и дизайнеры, и разработчики могут быстро оценить, насколько отличается 
новый дизайн от существующих решений и, как команда, решат стоит ли создавать 
что то заново или проще изменить то, что уже есть. Когда у вас нет такого 
руководства, это невозможно, что, по моему опыту, обычно значит что стили будут 
писаться по новой, а CSS — разрастаться.

В-третьих, так проще поддерживать целостность дизайна, так как дизайнер может 
увидеть все компоненты сайта в одном месте и убедится, что они выглядят и ведут
...
что бы описать необходимые изменения. И это значит, что у нас начисто отсутствует
...
результате элементам, но с помощью него можно отловить проблемы возникающие в 
различных браузерах. Отловив эти проблемы на ранних стадиях, можно избежать
возни с ними при дальнейшем тестировании.


## Шаги создания своего руководства по стилю

Далее я разберу процесс создания руководства по стилю, опираясь на мои первые
пару неделю в Editorially. (Потому что когда я работают в проекте без такого
...
с разметкой и добавил стили приложения, что бы любые изменения в CSS автоматически
отразились на руководстве.

Если посмотреть на руководство по стилю Yelp, вы увидите, что оно начинается с
...
конкретную разметку каждого такого элемента в руководство. 
...
Проанализируйте сайт и добавьте те паттерны, которые сочтете нужным добавить. 
В вашем проекте определенные лаяуты используются снова и снова, повторяются 
...
по стилю. Дайте им его посмотреть и они, надеюсь, помогут добавить все 
паттерны и модули, которые в нем нужны. Не забудьте, что *вся команда* должна 
участвовать, ведь это справочник для каждого её члена.
...
команде видеть, что это не просто статика, но и анимации. Так что когда ты
...
Потребуется добавить в таблицы стили комментарии или YAML, но вы сэкономите 
время в дальней перспективе, такой подход очень, очень упрощают поддержку. К 
тому же A List Apart выложил свою библиотеку паттернов на GitHub и посвятил её 
созданию [пост в блоге][12], показывая ещё один способ создания руководства по 
стилю. Ваши возможности намного шире, чем те, что я описал здесь; осмотритесь, 
что бы найти то, что будет наиболее полезно для вас и вашей команды.
...
поможет ему сориентироваться при разработке сайта, ведь он может так много 
рассказать и о представлении, и о коде, который его реализует.

Пока вы анализируете сайт или приложение, руководство никогда не будет по 
настоящему закончено. Но создав какую то документацию заранее, показав команде и 
получив от неё отзывы может быть весьма полезно. Привлекая всю команду к 
созданию руководства вы делаете этот процесс более *командным*, благодаря этому 
каждый станет вкладывает что то в его поддержку и начинает пользоваться 
руководством на регулярной основе.

Мы выложили руководство Editorially в публичный репозиторий на [GitHub][14] и 
сделали его доступным [онлайн][15]. Оно находилось на ранней стадии разработки и 
представляло собой результат работы всей команды, так что там была масса 
заметок, паттернов и бардака. Причина выложить его — желание показать тот факт, 
что руководство по стилю не должно выглядеть идеально. Несмотря на бардак, все 
это оказалось чрезвычайно полезно для меня и моей команды, когда мы продолжили 
разработку приложения.

Ну что, убедил? Хотели бы руководство по стилю для своего сайта или приложения?
Это стоит усилий: выделите время, соберите команду и начинайте — вы будете
вознаграждены документом, который ускорит процесс обсуждения и разработки.
